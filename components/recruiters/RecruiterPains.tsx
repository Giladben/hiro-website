const pains = [
  { before: 'מאות קו"ח שמגיעים ידנית ודורשים סינון ארוך', after: 'AI מדרג ומסנן לפי התאמה סמנטית — רואים רק את המובילים', icon: '📥' },
  { before: 'מועמדים טובים לא מחפשים עבודה אקטיבית', after: 'Job Sonar מאתר מועמדים פסיביים לפני שהם בשוק', icon: '🔍' },
  { before: 'CRM מפוצל, מיילים, ספרדשיטים ואין תמונה אחת', after: 'CRM אחד עם כל התקשורת, הסטטוסים והתיאומים', icon: '🗂️' },
  { before: 'פנייה ידנית לכל מועמד — גוזל שעות מדי יום', after: 'שליחת SMS ומיילים בתפוצה עם תבניות AI מותאמות', icon: '⚡' },
]

export function RecruiterPains() {
  return (
    <section
      aria-labelledby="rec-pains-heading"
    ...
    >
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <h2 id="rec-pains-heading" style={{ fontSize: 'clamp(1.7rem, 3vw, 2.4rem)', fontWeight: 800, color: 'var(--text)', lineHeight: 1.2 }}>
            גיוס ידני שבור — Hiro מתקנת אותו
          </h2>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
          {pains.map(p => (
            <div key={p.before} className="bento-card" style={{ padding: '1.5rem' }}>
